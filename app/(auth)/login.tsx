...
import { View, Text, StyleSheet, SafeAreaView, TouchableOpacity, KeyboardAvoidingView, Platform, ScrollView, Alert, AppState } from 'react-native';
  ...
  const handleLogin = async () => {
    ...
    if (isEmailValid && isPasswordValid) {
      try {
        ...
        if (authError) {
          Alert.alert(
            t('loginError') || 'Error de inicio de sesión',
            authError.message
          );
          return;
        }
        ...
        if (!authData.user) {
          Alert.alert(
            t('loginError') || 'Error de inicio de sesión',
            t('noUserDataReceived') || 'No se recibieron datos del usuario'
          );
          return;
        }

        // Call the store login method to sync the user data
          ...
          if (isFromTipFlow) {
            ...
            router.push({
              ...
              params: { 
                ...
                profilePicture: undefined // No profile picture available from login flow
              }
            });
          } else {
        ...
        Alert.alert(
          t('loginError') || 'Error de inicio de sesión',
          error instanceof Error ? error.message : (t('loginFailed') || 'No se pudo iniciar sesión. Por favor intenta de nuevo.')
        );
      }
    }
  };
