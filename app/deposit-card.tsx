...
export default function DepositCardScreen() {
  ...
  const handleDeposit = async () => {
    ...
    if (!validateCardNumber()) {
      showAlert('Error', t('invalidCardNumber'));
      return;
    }
    
    if (!validateExpiryDate()) {
      showAlert('Error', t('invalidExpiryDate'));
      return;
    }
    
    if (!validateCvv()) {
      showAlert('Error', t('invalidCVV'));
      return;
    }
    
    if (!validateCardholderName()) {
      showAlert('Error', t('nameRequired'));
      return;
    }
    
    if (!validateAmount()) {
      showAlert('Error', t('invalidAmount'));
      return;
    }
    ...
    try {
      ...
      const { url } = await apiClient.payments.createStripeCheckout({
      ...
      if (url) {
        ...
        showAlert('Error', 'No se pudo iniciar el pago');
      }
    } catch (error) {
      console.error('Deposit error:', error);
      showAlert('Error', 'No se pudo iniciar tu pago. Por favor intenta de nuevo.');
    }
  };
...
const styles = StyleSheet.create({
  ...
  scrollContent: {
    padding: '4%',
    paddingTop: 100, // Увеличили отступ для расстояния от кнопки назад
  },
  backButton: {
    position: 'absolute',
    top: 50,
